...
Sounds = {
	...
	Critical = Handle:WaitForChild("critical")
}
	...
	for i,v in pairs(character:GetChildren()) do
		if v:IsA("Tool") then
			if table.find(Swords,v.Name) then
				if v:FindFirstChild("Block").Value ~= false then
					print("Yes")
					return true
				else
					print("False")
	...
	if not tag and Tool.Enabled == false then
			if CheckSwordBlock(character) == false then
				if math.random(1,10) == 10 then
					...
					if not character:FindFirstChild("Bleeding") then
						local bleed = script.Bleeding:Clone()
						bleed.Parent = character
						bleed.Disabled = false
					else
						character:FindFirstChild("Bleeding"):FindFirstChild("Time").Value += 10
					end
				else
				...
				if math.random(1,4) == 4 then
					...
					print("False")
					Handle.Particle:FindFirstChild("sparkks"):Emit(60)
					Sounds.Deflect:Play()
					TagHumanoid(humanoid)
				end
			end	
